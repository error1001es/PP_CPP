...
inline double funTenCC(string line, int base_num = 2) { // перевод в 10 с.с.
...
inline string funNewCC(int num, int base_num = 2) { // перевод в нужную с.с.
...
inline void s4isl() {
    //int x = to_string(SafeNumberInput::Int("Введите число: "));
    ...
    int old_ss = SafeNumberInput::Int("Введите старую cистему счисления: ");
    int new_ss = SafeNumberInput::Int("Введите новую cистему счисления: ");


    int num = funTenCC(x, old_ss);
    cout << "Результат: " << funNewCC(num, new_ss) << endl;
}